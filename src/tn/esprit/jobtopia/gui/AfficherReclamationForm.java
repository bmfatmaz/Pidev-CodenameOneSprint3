...
public class AfficherReclamationForm extends Form {
    public AfficherReclamationForm(Resources res) {
        setTitle("Votre liste de réclamations");
        
        setLayout(new BoxLayout(BoxLayout.Y_AXIS));

        ServiceReclamation service = new ServiceReclamation();
        ArrayList<Reclamation> reclamations = service.afficherReclamations();

        for (Reclamation reclamation : reclamations) {
            Container container = new Container(new BoxLayout(BoxLayout.X_AXIS));

            // Title and date
            Container titleAndDateContainer = new Container(new BoxLayout(BoxLayout.Y_AXIS));
            titleAndDateContainer.add(new Label("Titre:"+reclamation.getTitre()));
            //titleAndDateContainer.add(new Label("Date de création:"+reclamation.getDate()));
            container.add(titleAndDateContainer);

            // Type and state
            Container typeAndStateContainer = new Container(new BoxLayout(BoxLayout.Y_AXIS));
            typeAndStateContainer.add(new Label("Type:"+reclamation.getType()));
            typeAndStateContainer.add(new Label("Etat:"+reclamation.getEtat()));
            container.add(typeAndStateContainer);

            // Description
            Container descriptionContainer = new Container(new BoxLayout(BoxLayout.Y_AXIS));
            descriptionContainer.add(new Label("Description:"+reclamation.getDescription()));
...
//        ArrayList<Reclamation>list = ServiceReclamation.getInstance().afficherReclamations();
//        
...
//
//        Container page1 = LayeredLayout.encloseIn(imageScale,overLay,BorderLayout.south(
...
//                if(ServiceReclamation.getInstance().supprimerReclamation(rec.getId())) {
